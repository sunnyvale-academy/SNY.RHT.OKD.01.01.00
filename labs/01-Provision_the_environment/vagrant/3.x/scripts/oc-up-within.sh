...
if [ "$(version $openshift_release)" -gt "$(version 3.7)" ]; then
    ...
    ansible-playbook --key-file "~/.ssh/id_rsa" -i /vagrant/ansible-hosts /home/vagrant/openshift-ansible/playbooks/deploy_cluster.yml
else
    ansible-playbook --key-file "~/.ssh/id_rsa" -i /vagrant/ansible-hosts /home/vagrant/openshift-ansible/playbooks/byo/config.yml
fi